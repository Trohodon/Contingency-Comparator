...
class LineRatingApp(tk.Tk):
    ...
    def _build_ui(self) -> None:
        ...
        self.input_vars = {
            ...
            "wind_speed_mps": tk.StringVar(value="0.6096"),   # 2 ft/s
            "wind_angle_deg": tk.StringVar(value="90"),       # beta = 0 in workbook
            "elevation_m": tk.StringVar(value="99.9744"),     # 328 ft
            "target_temp_c": tk.StringVar(value="100"),
            "emissivity": tk.StringVar(value="0.5"),
            "absorptivity": tk.StringVar(value="0.5"),
            "latitude_deg": tk.StringVar(value="30"),
            "line_azimuth_deg": tk.StringVar(value="90"),
            "line_voltage_kv": tk.StringVar(value="115"),
            "date_text": tk.StringVar(value="6/10/2015"),
            "time_text": tk.StringVar(value="4:00 AM"),
    ...
    def _rating_targets_for_conductor(self, conductor: Conductor, requested_max_temp_c: float) -> dict[str, float]:
        ...
        if kind == "CU-HYTHERM":
            return {
                "A": min(requested_max_temp_c, 119.0),
                "B": min(requested_max_temp_c, 125.0),
                "C": min(requested_max_temp_c, 150.0),
            }
        if kind == "ACCC":
            return {
                "A": min(requested_max_temp_c, 180.0),
                "B": min(requested_max_temp_c, 200.0),
                "C": min(requested_max_temp_c, 200.0),
            }
        if kind == "ACCR":
            return {
                "A": min(requested_max_temp_c, 210.0),
                "B": min(requested_max_temp_c, 240.0),
                "C": min(requested_max_temp_c, 240.0),
            }
        if kind == "ACSS":
            return {
                "A": min(requested_max_temp_c, 200.0),
                "B": min(requested_max_temp_c, 250.0),
                "C": min(requested_max_temp_c, 250.0),
            }
    ...
    def _populate_math_text(self, text_widget: tk.Text, payload: dict) -> None:
        ...
        given_lines = [
            ...
            f"Wind speed, Vw: {self._fmt(payload['wind_speed_mps'], 6)} m/s",
            f"Wind angle to conductor axis, phi: {self._fmt(payload['wind_angle_deg'], 3)} deg",
            f"Elevation, He: {self._fmt(payload['elevation_m'], 3)} m",
            ...
            f"Requested max conductor temperature: {self._fmt(payload['target_temp_c'], 3)} C",
        ]
        ...
        for rating_name in ("A", "B", "C"):
            ...
            self._insert_math_line(
                text_widget,
                self._bullet(
                    f"He = {self._fmt(payload['elevation_m'], 3)} m = {self._fmt(result['elevation_ft'], 6)} ft"
                ),
                ("body",),
            )
            self._insert_math_line(
    ...
    def _calculate_rating(self) -> None:
        ...
        try:
            ...
            result_lines = [
                ...
                f"  Wind     |  {wind_speed_mps:.3f} m/s @ {wind_angle_deg:.1f} deg",
                f"  Elev     |  {elevation_m:.1f} m",
